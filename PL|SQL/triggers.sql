CREATE OR REPLACE TRIGGER NOVOFUCIONARIO
 AFTER INSERT OR UPDATE ON EMPLOYEES
FOR EACH ROW 
WHEN (NEW.job_id = 'IT_PROG')
BEGIN
	update employees E set E.COMISSION_PCT = :NEW.SALARY * 0.3 where E.EMPLOYEE_ID = EMPLOYEE_ID
END;
--
CREATE OR REPLACE TRIGGER TGG_HIST_PREC 
AFTER UPDATE OF VALOR ON PRODUTOS
FOR EACH ROW
WHEN(OLD.VALOR <> NEW.VALOR)
BEGIN
	INSERT INTO Historico_Precos (idProduto, data, valor_antigo, valor_novo, usuario_bd, usuario_sistema) 
    VALUES (:OLD.idProduto, SYSDATE, :OLD.VALOR, :NEW.VALOR, USER, USER);
END;
